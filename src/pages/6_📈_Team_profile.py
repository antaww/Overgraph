...
def display_page_infos():
    ...
    st.markdown("""
                        ...
                        The profile includes various stats for the team, win rates, and performance over time.
                        This in-depth analysis could be crucial for team management and strategy planning.
                    """)
...
def double_y_lines(dataframe: pd.DataFrame, title: str, x: str, y1: str, y2: str, colory1: str, colory2: str,
    ...
    figure.add_trace(go.Scatter(x=[dataframe[x].min(), dataframe[x].max()], y=[mean_y1, mean_y1], mode="lines",
                                line=dict(color=colormean, dash='dash'), name=f'Mean of {y1}'))
...
try:
    ...
    with viz_tab:
        st.subheader(f'{team}\'s {stat} and winrate analysis')
        try:
            if chart_datas.empty:
                st.error('No data available for this team and this stage (you should select another stat)')
            else:
                ...
                fig2 = double_y_lines(dataframe=chart_datas,
                                      title=f'{team}\'s Avg {stat} and winrate over time on all stages', x='Start Time',
                                      y1=f'Avg {stat}', y2='Winrate', colory1='red', colory2='blue', colormean='green',
